class interrupt_tx_full_test extends uart_base_test;
     `uvm_component_utils(interrupt_tx_full_test)

     uart_configuration uart_config;

     function new(string name = "interrupt_tx_full_test", uvm_component parent);
     ...
     virtual task run_phase(uvm_phase phase);
          ...
          phase.raise_objection(this);
               ...
               if (~rdata[0])
                    `uvm_error(get_type_name(), $sformatf("Interrupt tx fifo full is not assert"))
          #10ms;
          phase.drop_objection(this);
     endtask:run_phase

endclass: interrupt_tx_full_test
